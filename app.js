...
{ 
  document.getElementById("txt-search").innerText= "";
  document.getElementById("results").innerHTML= "<div></div>";
  for( var i = 0; i < videosList.length; i++)
  {
   
    document.getElementById("results").innerHTML+= generateRowHTML(videosList[i]);
  }
}
...
function searchVideos()
{
    ...
    let wordPhrase = document.querySelector("#txt-search").value;
    ...
    else
    {
      ...
      let isWordPhraseFound = 0; //boolean value? TRUE means keyword exists. ?FALSE means keyword not found
      let isWordPhraseFoundFlag = 0; //isWordFound executes too quickly. As soon as a word is found, boolean becomes true
                                     //but if the word exists at one index, then does not exist at another index, boolean
                                     //becomes false again. this is a more permanent placeholder to maintain that the 
                                     //keyword was once found somewhere in the videoList array
      let wordPhraseNotFound = 0; 
      wordPhraseNotFound = 0; //values need to be reset to 0 after initialized to 0
      isWordPhraseFound = 0;
      isWordPhraseFoundFlag = 0;
      for(let k = 0; k < videosList.length; k++)
      {
        isWordPhraseFound = videosList[k].description.includes(wordPhrase);
        if(isWordPhraseFound === true)
        {
          document.getElementById("results").innerHTML+= generateRowHTML(videosList[k]);
          document.getElementById("txt-search").value="";
          isWordPhraseFoundFlag++;
        }
        else
        {
          wordPhraseNotFound++;
      ...
      if(wordPhraseNotFound > 0 && isWordPhraseFoundFlag == 0)
      {
        document.getElementById("results").innerHTML= "<div></div>";
        document.getElementById("txt-search").value="";
        document.getElementById("results").innerHTML+= `<p>No videos found with the keyword ${wordPhrase}</p>`;
      }
    }
